...
let console =
  ...
  in
  B0_jsoo.html_page "ocaml_console" ~requires ~doc ~srcs ~meta
...
let test_poke =
  ...
  B0_jsoo.html_page "test_poke" ~requires ~doc ~srcs ~meta
...
let test ?(requires = [brr]) n ~doc =
  ...
  B0_jsoo.html_page n ~requires ~doc ~srcs
...
let test_module ?doc top m requires  =
  ...
  let meta =
    B0_meta.empty
    |> ~~ B0_jsoo.compile_opts Cmd.(arg "--pretty")
    |> ~~ B0_show_url.url (`In (`Unit_dir, Fpath.(v test + ".html")))
  in
  B0_jsoo.html_page test ~requires ~doc ~srcs ~meta
...
let min =
  ...
  let requires = [brr] in
  B0_jsoo.html_page "min" ~requires ~doc:"Brr minimal web page" ~srcs

let nop =
  let srcs = [ `File ~/"test/nop.ml" ] in
  B0_jsoo.html_page "nop" ~doc:"js_of_ocaml nop web page" ~srcs
